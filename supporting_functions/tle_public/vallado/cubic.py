...
def cubic(a3, b2, c1, d0, opt):
    """
    Solve for the three roots of a cubic equation. There are no restrictions 
    on the coefficients, and imaginary results are passed out as separate values. 
    The general form is y = ax^3 + bx^2 + cx + d0. Note that r1i will always 
    be zero since there is always at least one real root.
    ...
    Args:
        ...
        d0: constant
        opt: option for output ('I' all roots including imaginary, 
             'R' only real roots, 'U' only unique real roots)
    ...
    if abs(a3) > small:
        ...
        # ------------------ use cardans formula ----------------------
        if delta > small:
            temp1 = (-b2 * 0.5) + np.sqrt(delta)
            temp2 = (-b2 * 0.5) - np.sqrt(delta)
            ...
            if abs(delta) < small:
                ...
                e0 = 2.0 * np.sqrt(-a3 * onethird)
                cosphi = (-b2 / (2.0 * np.sqrt(-a3 * a3 * a3 / 27.0)))
                sinphi = np.sqrt(1.0 - cosphi * cosphi)
                phi = np.arctan2(sinphi, cosphi)
                if phi < 0.0:
                    phi = phi + 2.0 * np.pi
                r1r = e0 * np.cos(phi * onethird) - p * onethird
                r2r = e0 * np.cos(phi * onethird + 120.0 / rad) - p * onethird
                r3r = e0 * np.cos(phi * onethird + 240.0 / rad) - p * onethird
    else:
