...
def quadric(a, b, c, opt):
    """
    Solve for the two roots of a quadric equation. There are no restrictions 
    ...
    if abs(discrim) < small:
        ...
        if abs(a) < small:
            ...
            if discrim > 0.0:
                r1r = (-b + np.sqrt(discrim)) / (2.0 * a)
                r2r = (-b - np.sqrt(discrim)) / (2.0 * a)
            else:
                # ------------------ complex roots --------------------
                if opt == 'I':
                    r1r = -b / (2.0 * a)
                    r2r = r1r
                    r1i = np.sqrt(-discrim) / (2.0 * a)
                    r2i = -np.sqrt(-discrim) / (2.0 * a)
                else:
                    r1r = 99999.9
                    r2r = 99999.9
